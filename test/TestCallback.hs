...
module TestCallback where

import Syn hiding (Context)
import Control.Concurrent.STM
import Control.Monad.IO.Class (liftIO)
import Data.Text (pack)
import Replica.Types (Context(Context, call, registerCallback))
import Prelude
import Replica.DOM as H 

import Replica.Events as P
...
route :: forall a b. Route a => Context -> a -> (a -> Syn HTML (AppUpdate a b)) -> Syn HTML b
route Context{call, registerCallback} initial f = do
  chan <- io newHistoryChan
  go initial chan
  where
    ...
    go :: a -> TChan String -> Syn HTML b
    go a chan = do
      r <- orr [ Left <$> f a, Right <$> liftSTM (readTChan chan) ]
      case r of
        Left (UpdateChangeUrl a') -> do
          io $ call (toRoute a') "window.history.pushState({}, \"\", arg);"
          go a' chan
...
routingApp :: State -> Syn HTML (AppUpdate State ())
routingApp = \case
  SiteA -> do
